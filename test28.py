...
class SimpleLogisticClassification:
    ...
    def fit(self, X, y, epochs=10, validation_data=None, test_data=None):
        ...
        for epoch in range(epochs):
            ...
            msg = ' loss = %8.4f, acc = %3.2f%%' % (train_loss, train_acc)

            if validation_data:
                print("if validation_data true " )
                val_loss = self.evaluate(validation_data[0], validation_data[1])
                val_acc = self.accuracy(self.predict(validation_data[0]), validation_data[1])
                msg += ', val_loss = %8.4f, val_acc = %3.2f%%' % (val_loss, val_acc)

            print(msg)

        if test_data:
            test_acc = self.accuracy(self.predict(test_data[0]), test_data[1])
            print('test_acc = %3.2f%%' % (test_acc))

    def accuracy(self, predictions, labels):
        return (np.sum(np.argmax(predictions, 1) == np.argmax(labels, 1))*100/predictions.shape[0])
